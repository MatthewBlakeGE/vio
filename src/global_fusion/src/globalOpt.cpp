...
void GlobalOptimization::inputOdom(double t, Eigen::Vector3d OdomP,
                                   Eigen::Quaterniond OdomQ) {
  mPoseMap.lock();
  vector<double> localPose{OdomP.x(), OdomP.y(), OdomP.z(), OdomQ.w(),
                           OdomQ.x(), OdomQ.y(), OdomQ.z()};
...
void GlobalOptimization::inputGPS(double t, double latitude, double longitude,
                                  double altitude, double posAccuracy) {
  double xyz[3];
...
void GlobalOptimization::optimize() {
  while (true) {
    if (newGPS) {
      ...
      TimeCost globalOptimizationTime;
        ...
        if (iterVIONext != localPoseMap.end()) {
          ...
          wTi.block<3, 3>(0, 0) =
              Eigen::Quaterniond(iterVIO->second[3], iterVIO->second[4],
                  ...
                  .toRotationMatrix();
          wTi.block<3, 1>(0, 3) = Eigen::Vector3d(
              iterVIO->second[0], iterVIO->second[1], iterVIO->second[2]);
          wTj.block<3, 3>(0, 0) =
        ...
        if (i == length - 1) {
          ...
          WVIO_T_body.block<3, 3>(0, 0) =
              Eigen::Quaterniond(localPoseMap[t][3], localPoseMap[t][4],
                  ...
                  .toRotationMatrix();
          WVIO_T_body.block<3, 1>(0, 3) = Eigen::Vector3d(
              localPoseMap[t][0], localPoseMap[t][1], localPoseMap[t][2]);
