...
bool InitialEXRotation::CalibrationExRotation(
  ...
  Rimu.push_back(
      delta_q_imu
          .toRotationMatrix());  //imu得到的旋转FIXME:这里和第七讲的L和R写反了
  ...
  for (int i = 1; i <= frame_count; i++) {
    ...
    ++sum_ok;
    Matrix4d L,
        R;  //L和R对应第七讲公式(6)中的左右四元数乘法left and right quaternion multiplication

    double w = Quaterniond(Rc[i]).w();
    Vector3d q = Quaterniond(Rc[i]).vec();
    L.block<3, 3>(0, 0) = w * Matrix3d::Identity() + Common::skewSymmetric(q);
    L.block<3, 1>(0, 3) = q;
    ...
    w = R_ij.w();
    q = R_ij.vec();
    R.block<3, 3>(0, 0) = w * Matrix3d::Identity() - Common::skewSymmetric(q);
    R.block<3, 1>(0, 3) = q;
