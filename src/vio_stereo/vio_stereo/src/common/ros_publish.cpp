...
void printStatistics(const Estimator& estimator, double t) {
  ...
  if (ESTIMATE_EXTRINSIC) {
    ...
    for (int i = 0; i < NUM_OF_CAM; i++) {
      ...
      ROS_DEBUG_STREAM(
          "extrinsic ric: " << Common::R2ypr(estimator.ric[i]).transpose());
...
void pubOdometry(const Estimator& estimator, const std_msgs::Header& header) {
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
    ...
    foutC << estimator.Ps[WINDOW_SIZE].x() << " "
          ...
          << estimator.Ps[WINDOW_SIZE].z() << " " << tmp_Q.x() << " "
          << tmp_Q.y() << " " << tmp_Q.z() << " " << tmp_Q.w() << endl;
...
void pubTF(const Estimator& estimator, const std_msgs::Header& header) {
  ...
  br.sendTransform(
      tf::StampedTransform(transform, header.stamp, "world", "base"));
  ...
  transform.setRotation(q);
  br.sendTransform(
      tf::StampedTransform(transform, header.stamp, "base", "camera"));
