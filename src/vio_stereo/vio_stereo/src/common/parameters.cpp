...
double INIT_DEPTH;
double MIN_PARALLAX;
double ACC_N, ACC_W, ACC_N_Z;
double GYR_N, GYR_W;
//滑动窗口中,是删除最老帧,还是倒数第二针,是通过新帧和倒数第二帧的视差决定,也就是这个最小像素值,还需要除以焦距
...
double
    SOLVER_TIME;  //max solver itration time (ms), to guarantee real time 最大求解器迭代时间（毫秒），以确保实时
...
void readParameters(std::string config_file) {
  FILE* fh = fopen(config_file.c_str(),
                   "r");  // r 以只读方式打开文件，该文件必须存在。
  if (fh == NULL) {
  ...
  CAM_NAMES.push_back(
      cam0Path);  //放的是含有相机参数的文件，因为有两个相机，所以用了vector
