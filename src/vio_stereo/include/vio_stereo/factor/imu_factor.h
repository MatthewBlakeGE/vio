  ...
  virtual bool Evaluate(double const* const* parameters, double* residuals,
    ...
    if (jacobians) {
      ...
      if (jacobians[0]) {
        ...
        jacobian_pose_i.block<3, 3>(O_P, O_R) = Common::skewSymmetric(
        ...
        jacobian_pose_i.block<3, 3>(O_R, O_R) =
            -(Common::Qleft(Qj.inverse() * Qi) *
              Common::Qright(corrected_delta_q))
                 .bottomRightCorner<3, 3>();
#endif

        jacobian_pose_i.block<3, 3>(O_V, O_R) =
            Common::skewSymmetric(Qi.inverse() * (G * sum_dt + Vj - Vi));
      ...
      if (jacobians[2]) {
        ...
        jacobian_pose_j.block<3, 3>(O_R, O_R) =
            Common::Qleft(corrected_delta_q.inverse() * Qi.inverse() * Qj)
                .bottomRightCorner<3, 3>();
#endif
