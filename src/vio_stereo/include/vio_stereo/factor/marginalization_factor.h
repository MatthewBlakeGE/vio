...
struct ResidualBlockInfo {
  ...
  ceres::CostFunction*
      cost_function;  //(其中parameter_block_sizes每个优化变量块的变量大小，以IMU残差为例，为[7,9,7,9])
  ...
  std::vector<
      ...
      jacobians;  //残差，IMU:15×1,视觉:2×1 VectorXd residuals;
...
class MarginalizationInfo {
 public:
  ...
  std::unordered_map<long, int>
      parameter_block_size;  // 需要marg掉的变量的大小,<优化变量内存地址,优化变量的长度>global size
  int sum_block_size;
  std::unordered_map<long, int>
      parameter_block_idx;  // <优化变量内存地址,在矩阵中的id>local size
  std::unordered_map<long, double*>
      parameter_block_data;  //<优化变量内存地址,数据>
  ...
  Eigen::MatrixXd
      linearized_jacobians;  //边缘化之后从信息矩阵恢复出来雅克比矩阵
  Eigen::VectorXd
      linearized_residuals;  //分别指的是边缘化之后从信息矩阵恢复出来残差向量
