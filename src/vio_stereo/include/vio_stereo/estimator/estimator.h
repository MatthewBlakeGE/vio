...
#include "factor/projection_1frame_2cam_factor.h"
...
class Estimator {
 public:
  ...
  queue<pair<double, map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>>>>
      featureBuf;
  double prevTime,
      curTime;  //这两个是按照哪个传感器的时间算的？ IMU时间，因为已经通过dt把相机时间转换到了IMU时间
  ...
  Preintegration* pre_integrations[(WINDOW_SIZE + 1)];  //里边放的是imu的预积分
  Vector3d acc_0, gyr_0;

  vector<double> dt_buf[(WINDOW_SIZE + 1)];
  vector<Vector3d> linear_acceleration_buf[(
      WINDOW_SIZE + 1)];  //加速度的预积分值,放的是划窗内每两帧之间的预积分值
  ...
  MarginalizationInfo*
      last_marginalization_info;  //上一时刻的先验信息,也就是上一个H矩阵matg掉一部分后剩下的内容
  vector<double*> last_marginalization_parameter_blocks;

  map<double, ImageFrame> all_image_frame;
  Preintegration* tmp_pre_integration;  //这个是输入到图像中的预积分值
