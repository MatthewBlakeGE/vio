...
FeatureTracker::trackImage(double _cur_time, const cv::Mat& _img,
  ...
  if (prev_pts.size() > 0) {
    ...
    ROS_DEBUG("temporal optical flow costs: %fms", t_o.time_end());
    //printf("track cnt %d\n", (int)ids.size());
  }
  ...
  if (1) {
    ...
    TimeCost t_m;
    setMask();
    ROS_DEBUG("set mask costs %fms", t_m.time_end());
    ...
    if (n_max_cnt > 0) {
      ...
      n_pts.clear();
    ROS_DEBUG("detect feature costs: %f ms", t_t.time_end());
...
void FeatureTracker::rejectWithF() {
  if (cur_pts.size() >= 8) {
    ...
    ROS_DEBUG("FM ransac costs: %fms", t_f.time_end());
  }
}
