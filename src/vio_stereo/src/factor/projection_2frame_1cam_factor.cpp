...
bool ProjectionTwoFrameOneCamFactor::Evaluate(double const* const* parameters,
  ...
  if (jacobians) {
    ...
    if (jacobians[1]) {
      ...
      jaco_j.rightCols<3>() =
          ric.transpose() * Common::skewSymmetric(pts_imu_j);
    ...
    if (jacobians[2]) {
      ...
      jaco_ex.rightCols<3>() =
          -tmp_r * Common::skewSymmetric(pts_camera_i) +
          Common::skewSymmetric(tmp_r * pts_camera_i) +
          Common::skewSymmetric(ric.transpose() *
