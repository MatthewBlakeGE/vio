...
void Estimator::inputImage(double t, const cv::Mat& _img,
  ...
  TimeCost featureTrackerTime;

  if (_img1.empty())
    featureFrame = featureTracker.trackImage(t, _img);
  else
    featureFrame = featureTracker.trackImage(t, _img, _img1);
  //printf("featureTracker time: %f\n", featureTrackerTime.time_end());
  ...
  if (MULTIPLE_THREAD) {
    ...
    featureBuf.push(make_pair(t, featureFrame));
    mBuf.unlock();
    TimeCost processTime;
    processMeasurements();
    printf("process time: %f\n", processTime.time_end());
  }
}
...
void Estimator::initFirstIMUPose(
  ...
  Matrix3d R0 = Common::g2R(averAcc);
...
void Estimator::processIMU(double t, double dt,
  ...
  if (!pre_integrations[frame_count]) {
    pre_integrations[frame_count] =
        new Preintegration{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};
  }
  if (frame_count != 0) {
    ...
    Rs[j] *= Common::deltaQ(un_gyr * dt).toRotationMatrix();
...
void Estimator::processImage(
  ...
  if (solver_flag == INITIAL) {
    ...
    ROS_DEBUG("solver costs: %fms", t_solve.time_end());
...
bool Estimator::visualInitialAlign() {
  ...
  Matrix3d R0 = Common::g2R(g);
  double yaw = Common::R2ypr(R0 * Rs[0]).x();
  R0 = Common::ypr2R(Eigen::Vector3d{-yaw, 0, 0}) * R0;
  g = R0 * g;
  ...
  ROS_DEBUG_STREAM("my R0  " << Common::R2ypr(Rs[0]).transpose());
...
void Estimator::double2vector() {
  Vector3d origin_R0 = Common::R2ypr(Rs[0]);
  Vector3d origin_P0 = Ps[0];

  if (failure_occur) {
    origin_R0 = Common::R2ypr(last_R0);
    origin_P0 = last_P0;
    failure_occur = 0;
  }
  ...
  if (USE_IMU) {
    Vector3d origin_R00 =
        Common::R2ypr(Quaterniond(para_Pose[0][6], para_Pose[0][3],
                                  para_Pose[0][4], para_Pose[0][5])
                          .toRotationMatrix());
    double y_diff = origin_R0.x() - origin_R00.x();
    //TODO
    Matrix3d rot_diff = Common::ypr2R(Vector3d(y_diff, 0, 0));
...
void Estimator::optimization() {
  ...
  TimeCost t_whole_marginalization;
  if (marginalization_flag == MARGIN_OLD) {
    ...
    TimeCost t_pre_margin;
    marginalization_info->preMarginalize();
    ROS_DEBUG("pre marginalization %f ms", t_pre_margin.time_end());

    TimeCost t_margin;
    marginalization_info->marginalize();
    ROS_DEBUG("marginalization %f ms", t_margin.time_end());
    ...
    if (last_marginalization_info &&
      ...
      marginalization_info->preMarginalize();
      ROS_DEBUG("end pre marginalization, %f ms", t_pre_margin.time_end());

      TimeCost t_margin;
      ROS_DEBUG("begin marginalization");
      marginalization_info->marginalize();
      ROS_DEBUG("end marginalization, %f ms", t_margin.time_end());
  ...
  //printf("whole marginalization costs: %f \n", t_whole_marginalization.time_end());
  ...
  if (marginalization_flag == MARGIN_OLD) {
    ...
    if (frame_count == WINDOW_SIZE) {
      ...
      if (USE_IMU) {
        ...
        pre_integrations[WINDOW_SIZE] = new Preintegration{
...
void Estimator::fastPredictIMU(double t, Eigen::Vector3d linear_acceleration,
  ...
  latest_Q = latest_Q * Common::deltaQ(un_gyr * dt);
